...
public class SimulationEngine {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        if (!graphManager.getNodes().containsKey("R5")) graphManager.addRouter("R5", "Admin_OPTIMIZATION", 2);
        ...
        RouterNode serverNode = graphManager.getNodes().get("R6");
        if (serverNode != null) {
            UserRequest unregisteredRequest = new UserRequest(serverNode, "Guest", "youtube.com");
            handleRequest(unregisteredRequest, loadBalancer, proxy, monitor, "Internet");
        }

        Random rand = new Random();
        String[] domains = {"facebook.com", "intranet.edu", "youtube.com", "reddit.com", "netflix.com", "email.edu", "twitch.tv"};
        String[] userTypes = {"Student", "Staff", "Guest"};
    ...
    private static void updateGraphDynamically(RouterGraphManager graphManager) {
        ...
        for (Edge edge : graphManager.getAllEdges()) {
            int newLatency = Math.max(1, edge.getLatency() + (rand.nextInt(11) - 5));
            int newCongestion = Math.max(0, edge.getCongestion() + (rand.nextInt(5) - 2));
        ...
        if (rand.nextBoolean() && nodeIds.size() > 3) {
        ...
        if (rand.nextBoolean() && currentEdges.size() > 1) {
    ...
    private static void handleRequest(UserRequest request, LoadBalancer loadBalancer, ProxySimulator proxy, TrafficMonitor monitor, String destination) throws InterruptedException {
        System.out.println("\n--- Handling Request ---");
        System.out.println("From: " + request.getSourceRouter().getId() + " (" + request.getSourceRouter().getType() + "), User: " + request.getUserType() + ", Domain: " + request.getDomain() + ", IP: " + request.getIpAddress());

        List<String> path = loadBalancer.getOptimalPath(request, destination);
        if (path.isEmpty()) {
            System.out.println("No path found" + (request.getIpAddress().equals("0.0.0.0") ? " (Unregistered user)." : "."));
            return;
        }
        ...
        if (!handled) {
            System.out.println("Request rerouted due to overload or unregistered user.");
            return;
        }
